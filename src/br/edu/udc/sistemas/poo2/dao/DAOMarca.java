...
public class DAOMarca extends DAO {
	...
	public void save(Object obj) throws Exception {
		...
		try {
			...
			if ((marca.getId() != null) && (marca.getId() > 0)) {
				sql = "update marca set descricao = '" + marca.getDescricao() + "' " + "where id = " + marca.getId();
	...
	public void remove(Object obj) throws Exception {
		...
		try {
			if ((marca.getId() != null) && (marca.getId() > 0)) {
				stmt = Database.getInstance().getConnection().createStatement();
				String sql = "delete from marca where id = " + marca.getId();
				System.out.println(sql);
				stmt.execute(sql);
			}
		} catch (Exception e) {
	...
	public Object[] find(Object obj) throws Exception {
		...
		try {
			...
			while (rst.next()) {
				...
				marcaResult.setId(rst.getInt("id"));
