...
public class DAOModelo extends DAO {
	...
	private Modelo validate(Object obj) throws Exception {
		if ((obj == null) || (!(obj instanceof Modelo))) {
			throw new Exception("Objeto n�o � uma Modelo!");
		}
		return (Modelo) obj;
	}
	...
	public void save(Object obj) throws Exception {
		...
		try {
			...
			if ((modelo.getId() != null) && (modelo.getId() > 0)) {
				sql = "update modelo set descricao = '" + modelo.getDescricao() + "'," + "idmarca = " + ((modelo.getMarca() != null) ? modelo.getMarca().getId() : "null") + " " + "where id = " + modelo.getId();
	...
	public void remove(Object obj) throws Exception {
		...
		try {
			if ((modelo.getId() != null) && (modelo.getId() > 0)) {
				stmt = Database.getInstance().getConnection().createStatement();
				String sql = "delete from modelo " + "where id = " + modelo.getId();
				System.out.println(sql);
				stmt.execute(sql);
			}
		} catch (Exception e) {
	...
	public Object[] find(Object obj) throws Exception {
		...
		try {
			stmt = Database.getInstance().getConnection().createStatement();
			String sql = "select id,idmarca,descricao from modelo";
			...
			while (rst.next()) {
				...
				modeloResult.setId(rst.getInt("id"));
